...
import {
  buildStructuralControlInventoryV2,
  materializeStructuralControlInventoryV2,
  planStructuralControlInventoryV2,
} from "./structural_control_inventory_v2.mjs";
...
test("fully verified exact hierarchy creates canonical v2 inventory without changing default processing", () => {
  const first = buildStructuralControlInventoryV2(verifiedInput());
  const second = buildStructuralControlInventoryV2(verifiedInput({
    generatedAt: "2026-08-25T01:00:00.000Z",
  }));

  assert.equal(first.status, "VERIFIED");
  assert.equal(first.inventory.schema_version, "opiu-structural-control-inventory.v2");
...
test("missing exact organization identity is honestly blocked with no selectable members", () => {
  ...
  const result = buildStructuralControlInventoryV2(input);
...
test("materializer binds exact verified inventory bytes and writes no binding for blocked inventory", async () => {
  ...
  try {
    const verified = await materializeStructuralControlInventoryV2({
    ...
    const verifiedAgain = await materializeStructuralControlInventoryV2({
      outputDirectory: verifiedDirectory,
    ...
    const stale = await materializeStructuralControlInventoryV2({
    ...
    blockedInput.hierarchyPeriods[0].status = "BLOCKED";
    const blocked = await materializeStructuralControlInventoryV2({
      outputDirectory: verifiedDirectory,
      ...blockedInput,
