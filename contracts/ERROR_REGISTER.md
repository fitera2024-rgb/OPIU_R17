...
### FY-001 — годовой период отклоняется и не материализуется помесячно в structural-control path

- Дата: `31.08.2026`
- Статус: `CONTRACTED`
- Сообщил: пользовательская карточка `FY001_ANNUAL_PERIOD_STRUCTURAL_SETTINGS`.
- Наблюдаемое поведение: исторический годовой запуск `run_df282d81be1b840dcb2d987d` завершился `R005_SETTINGS`; на exact base `cf639eacf5dcc232cf2d2ebc80780994f68e821b` Service-команда `materialize-structural-control-settings` воспроизводимо отклоняет `2025` ошибкой `BLOCKED_STRUCTURAL_CONTROL_SETTINGS_RUN_PERIOD_NOT_CONCRETE_MONTH:2025` до запуска R005.
- Ожидаемое поведение: годовой период `YYYY` обрабатывается как набор независимых concrete-month scopes `YYYY-01`…`YYYY-12`; существующий месячный `YYYY-MM` не меняется. Организация, период, physical evidence, provenance и `SourceRowID` связываются отдельно в каждом месяце; межмесячное и межорганизационное связывание и повторное использование физической ERP-строки запрещены.
- Затронутые пункты контракта: §§1.1, 3.1, 6.1, 10.1, 13–14; A01–A07, A10–A15. В частности, §10.1 требует раздельной сверки каждого конкретного периода, а годовой итог — суммы проверенных месяцев без взаимозачёта.
- Доказанная первая граница: `service/source/pipeline.go` передаёт год в `materializePackagedStructuralControlSettings`; `service/source/structural_control_pipeline_binding.go` запускает wrapper-команду materialization; `modules/reconciliation/source/service_r005_owner_wrapper.mjs` вызывает `materializeStructuralControlSettingsForRun`; `modules/reconciliation/source/structural_control_settings_binding.mjs` первым отклоняет год через `MONTH_PATTERN` / `RUN_PERIOD_NOT_CONCRETE_MONTH`. Граница совпадает с карточкой FY-001.
- Дополнительные блокировки, доказанные до изменения production-кода: после диагностического обхода только первой границы реальный R005 обработал все 12 месяцев и создал annual workbook, но сформировал `reconciliation.codex-input.json` размером `599626188` байт; wrapper завершился `RangeError: Invalid string length` при повторной сериализации. Даже при потоковой записи Service отклонит этот artifact по неизменяемому лимиту `128 MiB`. Одновременно materializer создал `structural-control-inventory.blocked.json` с `STRUCTURAL_INVENTORY_CONCRETE_MONTH_REQUIRED` и `STRUCTURAL_INVENTORY_EXACT_PERIOD_HIERARCHY_MISSING` для единого cross-month inventory.
- Вывод по scope: узкого изменения period parser/loader недостаточно для обязательного реального `R005 → evidence → Service handoff → R001`. Требуется отдельная разрешённая архитектурная materialization/fan-out годового Service-run на 12 month-owned R005 inventories/Codex artifacts/handoffs с безопасной агрегацией только итогов. Изменение wrapper serialization, Service artifact limits или ослабление concrete-month inventory guard в FY-001 запрещено как неподтверждённое расширение scope.
- Обязательные регрессионные тесты: `2025-10` остаётся принят; `2025` materializes 12 exact month scopes; `2025-13`, `25`, пустая строка и произвольный текст блокируются; каждый месяц сохраняет собственные organization/period/provenance/evidence; cross-month `SourceRowID` intersection и cross-org binding count равны `0`; posting/executed/live rows равны `0`, upload/execution/release/live 1C/delete равны `false`; полный Service-run доходит до R001 без `R005_SETTINGS` и без единого cross-month structural inventory.
- Реализация: `BLOCKED_SCOPE_EXPANSION_REQUIRED`; production-код не изменён.
- Протокол проверки: exact base SHA и contract SHA подтверждены; focused current settings tests `4/4 PASS`; first-boundary Service wrapper reproduction exit `1` с exact `RUN_PERIOD_NOT_CONCRETE_MONTH`; downstream diagnostic R005 обработал месяцы `2025-01`…`2025-12`, сохранил `REPORT_ONLY`, `posting_rows=0`, `execution_allowed=false`, `ready_to_upload=false`, `release_allowed=false`, `live_1c_allowed=false`, затем доказал три независимых annual blockers выше. Реальная годовая acceptance и полный regression не заявляются как PASS.
