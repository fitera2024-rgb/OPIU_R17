...
const INVENTORY_SCHEMA = "opiu-structural-control-inventory.v2";
const BINDING_SCHEMA = "opiu-structural-control-inventory-binding.v2";
...
function validateTree(tree, side, blockers) {
  ...
  const members = [];
  for (let sourceOrder = 0; sourceOrder < roots.length; sourceOrder += 1) {
    const rootID = roots[sourceOrder];
    const node = byID.get(rootID);
    if (!node) {
      addBlocker(blockers, blocker("ROOT_NODE_MISSING", side, { node_id: rootID }));
      continue;
    }
    if (node.is_group !== true) addBlocker(blockers, blocker("ROOT_NOT_GROUP", side, { node_id: rootID }));
    ...
    if (!name || !hierarchyPath) addBlocker(blockers, blocker("ROOT_LABEL_INCOMPLETE", side, { node_id: rootID }));
    if (amountCents === null) addBlocker(blockers, blocker("ROOT_AMOUNT_NOT_EXACT_CENTS", side, { node_id: rootID }));
...
export function buildStructuralControlInventoryV2({
  runId,
  contextId,
  organization,
  ...
  if (periodItem && text(periodItem.status) !== "PASS") {
    addBlocker(blockers, blocker("PERIOD_HIERARCHY_NOT_PASS", "RUN", { status: text(periodItem.status) }));
  }
  if (periodItem && text(periodItem.intalev_hierarchy_status) !== "PROVEN") {
    addBlocker(blockers, blocker("INTALEV_HIERARCHY_NOT_PROVEN", "INTALEV", {
      status: text(periodItem.intalev_hierarchy_status),
    }));
  }
...
export function planStructuralControlInventoryV2(input = {}) {
  const result = buildStructuralControlInventoryV2({
    ...input,
...
async function verifyCurrentRunFiles(currentRunFiles, input, plan) {
  ...
  for (const [role, artifact] of [["CODEX_INPUT", codexInput], ["MANIFEST", manifest]]) {
    ...
    if (!sameResolvedPath(document.output_path, report.file) ||
        text(document.output_sha256).toUpperCase() !== report.sha256) {
...
export async function materializeStructuralControlInventoryV2({
  outputDirectory,
  currentRunFiles,
  ...input
} = {}) {
  ...
  const plan = planStructuralControlInventoryV2(input);
...
export const STRUCTURAL_CONTROL_INVENTORY_V2_SCHEMA = INVENTORY_SCHEMA;
export const STRUCTURAL_CONTROL_INVENTORY_BINDING_V2_SCHEMA = BINDING_SCHEMA;
