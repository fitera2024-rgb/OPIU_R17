...
- Ожидаемое поведение: после запуска нового экземпляра Service все сохранённые `QUEUED`, `PREFLIGHT` и `RUNNING`, принадлежавшие уже завершившемуся процессу, атомарно переводятся в терминальный `FAILED / INTERRUPTED_SERVICE_RESTART` с понятным русским сообщением и временем завершения. Промежуточный XLSX или иной неполный артефакт не считается готовым результатом. Уже терминальные запуски не изменяются.
...
- Обязательный регрессионный тест: persisted `QUEUED`, `PREFLIGHT` и `RUNNING` при `OpenStore` становятся `FAILED / INTERRUPTED_SERVICE_RESTART`, получают непустое `finished_at` и прежний `REPORT_ONLY`; completed/blocked/failed остаются побайтово эквивалентны по бизнес-полям; наличие orphan XLSX не повышает статус; второе открытие хранилища идемпотентно; `hasActiveRuns=false`.
- Реализация: `OpenStore` под существующей durable-state блокировкой одним сохранением переводит только прежние `QUEUED`, `PREFLIGHT` и `RUNNING` в `FAILED / INTERRUPTED_SERVICE_RESTART`, ставит единое UTC-время завершения и понятное русское сообщение. Safety и остальные поля сохраняются; каталоги запуска не сканируются, поэтому orphan XLSX не получает статуса готового результата. Повторное открытие идемпотентно.
- Протокол проверки: focused recovery `-count=10` — PASS; recovery вместе с R001/R005 result guards `-count=3` — PASS; расширенный Store/OpenStore набор — PASS; `go vet ./...` и `git diff --check` — PASS. Независимый final review подтвердил exact scope, сохранение `REPORT_ONLY`, неизменность всех терминальных статусов и отсутствие изменений lifecycle/port/R005/R001/UI/financial/packaging.
