...
### R005-016 — успешная R005 теряется при downstream-ошибке R001

- Дата: `01.09.2026`
- Статус: `IMPLEMENTED_SOURCE_REGRESSION; PORTABLE_ACCEPTANCE_PENDING`
- Сообщил: пользовательская карточка `NIGHT_SAKHALIN_R005_RESILIENT_PIPELINE_AND_WORKING_EXE`.
- BUG / наблюдаемое поведение: exact-base Service валидирует и атомарно якорит R005 до запуска R001, а result API независимо выдаёт R005 после downstream-ошибки. Дефект воспроизводится в user-facing семантике: whole-run становится generic `FAILED/R001`, главная ссылка результа скрывается, а R001-карточка говорит «R001 ещё не готов» вместо terminal «не сформирован» с причиной. Для invalid/corrupt/timeout конкретная downstream-причина также теряется в persisted `Run.Message`.
- EXPECTED / ожидаемое поведение: после валидации R005 Service немедленно и атомарно регистрирует её файл, manifest и diagnostics как самостоятельный пользовательский результат. Downstream R001 success сохраняет обычный full-success; R001 failure/invalid output/timeout дает честный terminal-статус и причину, но не удаляет, не скрывает и не инвалидирует R005. R001 не маркируется готовым ложно. Если не удалась сама R005, reconciliation завершается ошибкой без фиктивного результа.
- EVIDENCE / RED exact base: первоначальная проверка только сохранности R005 была недостаточным RED, потому что уже проходила на base. Усиленный production-path RED фактически выполнен в detached worktree `C:\Users\NB-FIT\Documents\Codex\2026-09-01\files-pasted-by-the-user-task-11\work\OPIU_R17_R005_EXACT_BASE_RED` на exact SHA `1e42719d7e79efcbf1d40375396f24c44790be86`: `go.exe test -v -count=1 -run '^TestExactBaseR005DownstreamReasonRED$' .` завершился `FAIL` (`2.479s`). После `POST /api/runs → verified/anchored R005 → forced R001 exception` expected substring `R001 не сформирован: принудительная проверочная ошибка после успешной R005`, actual persisted message `R001 не сформировал полный безопасный диагностический комплект`. Полный воспроизводимый лог: `C:\Users\NB-FIT\Documents\Codex\2026-09-01\files-pasted-by-the-user-task-11\work\OPIU_R17_R005_EXACT_BASE_RED\R005_016_EXACT_BASE_RED.log`. Production-path invalid/corrupt R001 и `context.DeadlineExceeded` также требуют классифицированную причину, тогда как exact base сохраняет generic message. UI RED exact base показывает `Результат R001 / Нет результата / R001 ещё не готов` и скрывает top-level result access. Fresh user run `run_7de3e7aa23fb2b5785870935` не является свидетельством этого defect: он terminal `FAILED/R005` за `19.481s`, поскольку context привязал stale 9 УК sources; R005/R001 артефактов нет. Этот independent context defect зарегистрирован отдельно как `CONTEXT-001` на ветке `fix/sakhalin-context-binds-current-uploads`.
- EVIDENCE / GREEN source: на task-branch pinned Go `go1.22.12` focused production-path T2/T3/T5/T6, deferred collection/bootstrap guard и closed diagnostic mapping прошли `go test -count=1` (`5.613s`): R005 listing/direct workbook/manifest HTTP `200`, R001 `ready=false` и без файлов/archive, diagnostics содержит safe classified reason; `OpenStore` после anchored R005 переводит orphaned `RUNNING/R001` в `FAILED/INTERRUPTED_SERVICE_RESTART`, сохраняет anchor и verified R005 API/direct. Обычные `/api/runs` и `/api/bootstrap` не выполняют eager physical hashing; exact-run lookup и result route повторно валидируют и отклоняют tampered R005. Финальный focused `-count=50` после всех review-изменений — `PASS` (`272.726s`), intermittent failures `0`. Полный `go test -count=1 ./...` — `PASS` (`150.197s`), `go vet ./...` — `PASS`. Pinned Node `v24.14.0`: два R005-016 UI-файла `6/6 PASS`; focused вместе с existing `results-ui.test.cjs` — `7/7 PASS`; полный web suite после live UI race regression — `64/64 PASS`; UI overlay — `21/21 PASS`; R005 — `244 PASS / 1` объявленный opt-in skip; R001 — `268 PASS / 1` объявленный opt-in skip; оба October opt-in golden дополнительно прошли без skip. Production syntax — `99/99 PASS`; applicable portable packaging — `116 PASS / 2` объявленных environment skip; `git diff --check` — `PASS`. Independent reviewer — `CLEAN`. Это source evidence, не portable acceptance.
- ROOT CAUSE: на exact base единый whole-run status честно переходит в `FAILED/R001`, но `web/app.js` даёт главную ссылку только для `COMPLETED_REPORT_ONLY`, а `web/results-ui.js` не читает terminal `run.stage/status/message` и представляет failed R001 как in-progress. В invalid/corrupt/timeout ветках `pipeline.go` конкретная причина заменяется generic persisted message. Новый persisted enum не требуется.
- LIVE UI ACCEPTANCE DEFECT: первый packaged Run A `run_4780c693664360ccda185f0e` завершился `COMPLETED_REPORT_ONLY / DONE`, а API физически подтвердил R005 `ready=true` и три прямые ссылки. Однако `loadForItem` валидировал большой `reconciliation.codex-input.json` параллельно с очередным bootstrap-refresh; `renderRuns` заменил `.list-item`, и завершившаяся загрузка прикрепила кнопки скачивания к уже detached DOM-узлу. Пользователь снова видел «Показать результаты» без карточки. Исправление сохраняет только явно раскрытые run IDs и после bootstrap redraw неблокирующе восстанавливает их карточки через status-aware single-flight/cache; нераскрытые исторические результаты по-прежнему не запрашиваются. Исполняемый deferred-Promise DOM regression `web_tests/ui-007-result-redraw-race.test.cjs` доказывает оба порядка событий: same-status redraw переиспользует ровно одну R005/R001 пару и прикрепляет результат к replacement node; terminal redraw запускает свежую пару, а поздний RUNNING response не отравляет terminal cache. Повторная packaged UI-приёмка обязательна до закрытия portable gate.
- SCOPE: production orchestration `POST /api/runs → R005 validation/registration → handoff → R001 → finalization`, independent result API, terminal UI semantics, recovery после фиксации R005, focused production-path regressions и эта запись.
- OUT OF SCOPE: бухгалтерские golden-суммы Сахалина, ослабление physical-evidence/one-row-once/scope guards, новый Rules Service, автоматическая загрузка/проведение 1С, изменение approvals или lifecycle UI-013-RDL/A16.
- REGRESSION TEST: T1 R005 PASS+R001 PASS; T2 deterministic R001 exception; T3 invalid/corrupt R001; T4 R005 failure; T5 R001 timeout; T6 restart/recovery после persisted R005. Для T2/T3/T5 API обязан выдать terminal run, R005 result/manifest/direct file `200`, R001 NOT READY/FAILED, diagnostics с причиной; UI останавливает polling и показывает R005 ready отдельно от R001 error.
- PASS / текущая доказанная граница: source матрица T1–T6 и full regression зелёные; R005 повторно валидируется на exact/result границе, R001 остаётся NOT READY, произвольные diagnostic tokens/пути/secret-like строки не попадают в user message, safety остаётся `REPORT_ONLY`. Реальный Сахалин в portable EXE, A/B deterministic build, relocation 2/2 и violations=`0` ещё не доказаны этой записью и не заявляются PASS до отдельного acceptance evidence.
