...
try {
    ...
    $uiPath = Join-Path $PSScriptRoot 'correction_ui.ps1'
    if (-not (Test-Path -LiteralPath $uiPath -PathType Leaf)) {
        throw "Не найдено рабочее окно R001: $uiPath"
    }
    ...
    $message = "Рабочее окно R001 не открылось.`r`n`r`n$($_.Exception.Message)`r`n`r`nДиагностика: $logPath"
    ...
    if ($ReadyPath) {
        try {
            ...
            [ordered]@{ status = 'ERROR'; module = 'R001'; message = $_.Exception.Message; timestamp = [DateTime]::UtcNow.ToString('o') } | ConvertTo-Json -Depth 3 | Set-Content -LiteralPath $ReadyPath -Encoding UTF8
        } catch { }
    }
    try {
        Add-Type -AssemblyName System.Windows.Forms
        [System.Windows.Forms.MessageBox]::Show($message, 'Корректировки ОПИУ R001', 'OK', 'Error') | Out-Null
    }
    catch {
