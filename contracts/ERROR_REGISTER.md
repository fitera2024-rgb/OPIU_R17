...
### R005-026 — ERP target path сокращается до статьи без authoritative parent-chain

- Дата: `02.09.2026`
- Статус: `IMPLEMENTED`
- Сообщил: пользовательская карточка `R005-026_FULL_CANONICAL_ERP_TARGET_PATH`.
- Наблюдаемое поведение: parser authoritative ERP-каталога присоединяет entry к текущей outline-ветви, но затем synthetic target node формирует `full_path` только из блока и имени статьи. Для `00-000121`, счёт `44.1`, физическая цепочка `Коммерческие расходы / ФЗП и компенсационные выплаты / НДФЛ / НДФЛ` сокращается до `Коммерческие расходы / НДФЛ`; аналогично теряется промежуточный родитель у незарплатных entry.
- Ожидаемое поведение: generic target resolver возвращает точный canonical physical node и полный доказанный parent-chain authoritative ERP-каталога. Одноимённая статья под другим родителем не взаимозаменяема. Missing, ambiguous, cyclic, disconnected или конфликтующая authority, включая расхождение outline-parent и явной parent-колонки, остаётся fail-closed с безопасной диагностикой; label не угадывается и fuzzy matching запрещён.
- Доказательство: authoritative packaged catalog `modules/reconciliation/source/resources/СтатьиДоходовИРасходовЕРП.xlsx`, лист `Лист_1`, SHA-256 `8C7CA0E3884BB78F610A95E01A935DEBF55871312D3193728B248E7C4A197BBF`; entry row `287`, code `00-000121`, account `44.1`; outline parents row `286` `НДФЛ`, row `281` `ФЗП и компенсационные выплаты`, row `158` `Коммерческие расходы`. Незарплатные контрольные entries: row `106`, `00-000067`/`26`, path `Административные расходы / Расходы на персонал / Проезд/доставка сотрудников`; row `468`, `00-000158`/`44.2`, path `Расходы на складскую логистику / Содержание складов / Страхование имущества`.
- Затронутые пункты контракта: §§4.1–4.3, 6–7, 9.3–9.6, 10.1, 12.2–12.4, 13–14; A07, A08, A10, A17, A21, A22, A32. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A`, не изменяется.
- Обязательный RED: focused fixture доказывает текущее сокращение exact entry path; отдельный незарплатный entry сохраняет все промежуточные родители. Negative fixtures блокируют одноимённую статью под другим parent-chain, missing/unproven chain и disagreement authority. Movement `153`, financial target selection и main reconciliation tree сохраняют прежние результаты.
- Допустимый scope: parser/normalizer authoritative ERP article catalog, generic exact physical-node parent-chain resolver, прямые потребители canonical ERP target path, focused regression и эта запись. NDFL/payroll/code/row hardcodes, fuzzy match, guessed labels, изменение financial target selection, movement `153`, R001 и main-tree semantics запрещены.
- PASS: exact entry получает полный physical parent-chain с code/account/row provenance; disagreement/missing/ambiguous authority не выбирает путь; same label under another parent не подменяет цель; незарплатный fixture проходит тем же generic resolver; Sakhalin `ERP-000000076 / 3 Сахалин / 2025-01` сохраняет physical ERP row, SourceRowID, сумму, economic target, balanced STORNO/REPOST и safety invariants. `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live rows `0`, execution/upload/release/live-1C flags `false`.
- Реализация: отдельный generic resolver принимает только физически текущий contiguous outline stack exact entry, сверяет каждый level/path и равенство immediate outline parent со статьёй entry, сохраняет полный parent-chain и top economic block как разные поля. Missing/discontinuous/disagreeing authority возвращает явный `BLOCKED_ERP_ENTRY_*` diagnostic и не создаёт exact target node. Финансовый rank и target selector не изменялись; hard-coded article/code/account/row labels и fuzzy matching отсутствуют.
- Протокол проверки: обязательный focused RED до production mutation — `0/1 PASS`, `ERR_MODULE_NOT_FOUND`; regression одновременно фиксирует legacy actual `Коммерческие расходы / НДФЛ` при expected полном пути. Focused GREEN — `6/6 PASS`; directly affected presentation/cross-journal/R005-024/main-tree — `52/52 PASS`; полный последовательный R005 Node — `267 tests / 266 PASS / 1 existing opt-in SKIP / 0 FAIL`; syntax изменённых production/test modules и `git diff --check` — `PASS`.
- Fresh Sakhalin smoke: isolated owner-wrapper run `run_r005026_20260902_b`, `ERP-000000076 / 3 Сахалин / 2025-01`, сформировал workbook SHA-256 `B3B029569B9296B08D95AE3286E254D4439A879B0F75CF5E845D4691B51940F8`. Коммерческий payroll NDFL сохранил `26` `PAYROLL_COMPONENT_PAIR`, сумму `442954`, те же `26` ERP SourceRowID, article `НДФЛ`, code `00-000121`, account `44.1`, economic block `Коммерческие расходы`, а `target_catalog_path` стал `Коммерческие расходы / ФЗП и компенсационные выплаты / НДФЛ`; в `04A_Расхождения_проводок` полный путь присутствует `26` раз, сокращённый — `0`.
- Инварианты smoke: до/после `unique_pairs=330`, `approved_balanced_pairs=38`, `financial_pair_rows=76`, reused Intalev/ERP `0/0`, posting/live/executed `0/0/0`. Movement `153` отдельно сохранил `Налоги / НДФЛ`, `352762`, `68.2 / 51`, `BLOCKED_TARGET_INPUT`, target code/account/path пусты. Main tree до/после: `S074 parent=S001 outline=1`, `S075 parent=S074 outline=2`, `S077 parent=S075 outline=3`; строки `126/127/130` и суммы неизменны. Safety: `REPORT_ONLY=true`, `rules_service=false`, execution/upload/release/live/delete `false`.
