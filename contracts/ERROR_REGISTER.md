...
- Safety остаётся неизменной: `REPORT_ONLY=true`, `rules_service=false`, `posting_rows=0`, `executed_posting_rows=0`, `live_posting_rows=0`; автоматическая загрузка или проведение в 1С запрещены; финансовая строка требует физического ERP evidence; повторное финансовое использование одной физической строки ERP запрещено.
...
### R005-022 — область физической организации не учитывает доказанное членство в ERP-иерархии

- Дата: `02.09.2026`
- Статус: `VERIFIED`
- Сообщил: пользовательская карточка `R005-022_PHYSICAL_ORGANIZATION_HIERARCHY_SCOPE` и Issue #30.
- Наблюдаемое поведение: для области `ERP-000000076 / 3 Сахалин`, период `2025-01`, доказанная физическая строка ERP `1617` с `SourceRowID=2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825`, организацией `ПВ` и суммой `1854` отклоняется как `PHYSICAL_ORGANIZATION_SCOPE_MISMATCH`, потому что production gate получает только выбранное имя организации и статические aliases.
- Ожидаемое поведение: допустимый периметр физических организаций выводится из authoritative ERP-иерархии точного выбранного корня. Узел физической организации должен быть доказан как организация/entity и как потомок этого корня; одноимённый узел другого корня, подразделение, ЦФО, presentation-узел, отсутствующая либо неоднозначная hierarchy proof остаются fail-closed.
- Доказательство: реальный smoke `run_r005021_20260902_105403`; authoritative path `3 Сахалин / Торговая компания / ПВ`; физическая строка `1617`, сумма `1854`, статья `Проезд/доставка сотрудников`, документ `Трансляция 0000001782 от 19.03.2026 9:03:39`, Дт `26`, Кт `76.5`.
- Подтверждённая первопричина: `allowedPhysicalOrganizations` строится из `[selected organization, ...profile.journalOrganizationAliases]`; статические текстовые aliases не являются authority физического организационного периметра и не включают доказанные organization/entity-потомки выбранного ERP-корня.
- Затронутые пункты контракта: §§3.1–3.2, 4.2, 6–7, 9.3–9.6, 10.1, 12.2–12.4, 13–14; A06–A08, A10, A21–A22, A25, A30–A32.
- Обязательный регрессионный тест: `ПВ` как доказанный organization/entity-потомок `3 Сахалин` проходит; `ЧУЖАЯ` из `4 Владивосток`, одноимённый узел другого корня, department/ЦФО, missing и ambiguous hierarchy proof отклоняются; exact period, SourceRowID, amount, uniqueness и reuse guards остаются неизменными.
- Допустимый scope: узкий helper hierarchy membership/organization identity, уже используемая authoritative ERP-иерархия, call site формирования `allowedPhysicalOrganizations`, focused tests и эта запись. Hard-coded `ПВ`, добавление alias, fuzzy matching, R001, owner economic route proofs и независимые структурные блокеры запрещены.
- PASS: реальный SourceRowID `2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825` сохраняет строку `1617`, организацию `ПВ`, сумму `1854` и доказанное членство под `3 Сахалин`; `PHYSICAL_ORGANIZATION_SCOPE_MISMATCH` для этой строки отсутствует. Следующий независимый blocker, если появится, только фиксируется. `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live rows `0`, execution/upload/release/live-1C flags `false`.
- Уточнение authority: текущий pinned `ОрганизациииерархияЕРП.xlsx`, SHA-256 `3342603C0782FE12871AD55E7E19E778A97651E8CFF2E00F0CE6774295C57522`, не содержит пригодного явного `entity_type`; `node_type=ORGANIZATION` также установлен у подразделений. Поэтому положительное доказательство descendant entity требует verified parent-id chain до exact id/name/path root, стабильной identity, authoritative `metadata.inn` и глобально однозначного имени. Legacy `entity_type=Налоговый резидент РФ` из другого реестра authority этой задачи не является.
- Реализация: новый fail-closed helper проверяет SHA фактического XLSX и provenance нормализованного `organizations.json`, exact root и каждую parent/depth/path/top chain; допускает только root и доказанные legal-entity descendants. Production call site передаёт в неизменённый A22 gate только `physicalOrganizationScope.member_names`; `journalOrganizationAliases` больше не формирует финансовый физический периметр.
- Протокол проверки: обязательный RED до production helper — `ERR_MODULE_NOT_FOUND`, `0/1 PASS`; focused GREEN — `6/6 PASS`, включая реальный pinned ERP-каталог, foreign root, department, duplicate name, missing/malformed proof и прежние period/SourceRowID/amount/uniqueness/reuse/balance guards. Полностью затронутые test files — `50/50 PASS`; syntax checks helper и production — `PASS`; `git diff --check` — `PASS`.
- Реальный Sakhalin smoke: `run_r005022_20260902_114209` на тех же Intalev/ERP ZIP и `2025-01` доказал perimeter `3 Сахалин`, `ООО "Первый хладокомбинат"`, `ООО "Планета Витаминов"`, `ПВ`; у целевого SourceRowID строка `1617`, range `B1617:AG1617`, организация `ПВ`, сумма `1854`, gate `ДОКАЗАНО / UNIQUE_PHYSICAL_ERP_ROW`, STORNO/REPOST `-1854/+1854`, posting/live/executed `0`.
- Следующий независимый blocker: `SOURCE_ROW_NOT_FOUND` в `source_driven_expense_coverage.journal_first_attribution.unresolved[19]` для того же SourceRowID. Он не исправлялся в R005-022 и не отменяет доказанную A22 REPORT_ONLY-пару. Safety остаётся `REPORT_ONLY`, posting/live/executed `0`, execution/upload/release/live-1C `false`.

### R005-024 — live-report placement подменяет экономический блок физического счёта Инталев

- Дата: `02.09.2026`
- Статус: `VERIFIED`
- Сообщил: пользовательская карточка `R005-024_INTALEV_ACCOUNT_ECONOMIC_BLOCK_AUTHORITY`.
- Наблюдаемое поведение: enrichment физической строки Инталев записывает `article_info.block = reportPlacement.block || articleInfo.block`. Поэтому presentation-путь живого отчёта может подменить блок, доказанный физическим счётом расхода, и стать финансовой authority выбора целевой ERP-статьи.
- Ожидаемое поведение: финансовый economic block определяется только признанным расходным счётом физической строки Инталев: `26 => Административные расходы`, `44.1 => Коммерческие расходы`, `44.2 => Расходы на складскую логистику`, `44.3 => Расходы на транспортную логистику`. Расходная сторона может быть Дт или Кт. Live-report placement сохраняется отдельно только как presentation/diagnostic evidence. Если признанного расходного счёта нет либо Дт/Кт указывают на конфликтующие расходные блоки, financial target остаётся blocked/review и не выводится из дерева отчёта.
- Реальное доказательство: fresh accepted run `run_bfe88b9b2b846934dd091ba3`; Intalev `SourceRowID=49F314B1C167A69C21DAB409F8226D71FEAC1B1B2517848408888842D07A272A`, физическая строка `4082`, `31.01.2025 0:00:00`, документ `Проформа 00000000293 от 31.01.2025 8:31:03`, Дт `26`, Кт `76`, сумма `1854`, ЦФО `ЦМД Сахалин`, статья/аналитика `Прочие административные расходы / Операционист / Проезд/доставка сотрудников / Январь`, содержание `Антипина 445р., Накаябу 791, Деркунская 618р. Такси 29.01`. Физический счёт `26` доказывает `Административные расходы`, тогда как `reportPlacement.block=Расходы на складскую логистику`; до override `articleInfo.block=Административные расходы`, после override текущий `article_info.block=Расходы на складскую логистику`. Конфликт authority доказан независимо агентами A и C.
- ERP-пара: `SourceRowID=2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825`, физическая строка `1617`, сумма `1854`. Текущая ошибочная authority выбрала warehouse-цель `Расходы на складскую логистику / Проезд/доставка сотрудников`, код `ЦБ-000279`, счёт `44.3`.
- Подтверждённая первопричина: `cross_journal_discrepancy_evidence.mjs` смешивает два разных смысла в `article_info.block`; затем `selectTargetArticle`, approval scope и финансовые поля потребляют уже переопределённый presentation-блок как экономический.
- Затронутые пункты контракта: §§4.1–4.3, 6–7, 9.1–9.6, 10.1, 12.2–12.4, 13–14; A07, A08, A10, A17, A21, A22, A32. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A` не изменяется.
- Обязательный RED на exact base `d03fc233179d4274cc82c76bcffce749de312ba2`: focused `r005_intalev_account_economic_block_authority.test.mjs` — `0/5 PASS`. Debit `26`, debit `44.1` и credit-side `44.3` ошибочно получили warehouse target; no-expense и conflicting-expense fixtures ошибочно получили `PROVEN_UNIQUE_TARGET_IN_INTALEV_BLOCK` вместо `BLOCKED_TARGET_INPUT`.
- Допустимый scope: явное разделение `economic_block` и `presentation_block` в cross-journal evidence, потребители financial target/scope этого блока, focused regression и эта запись. R001, R005-021, R005-022, owner economic-route proofs и `journal_first_presentation_attribution` не изменяются.
- PASS: target lookup выполняется только внутри account-derived economic block; presentation block остаётся наблюдаемым в диагностике и не меняет финансовую цель; no-expense/conflict остаются fail-closed; exact physical ERP `SourceRowID` и no-reuse guards сохраняются. Safety неизменна: `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live rows `0`, `execution_allowed=false`, `ready_to_upload=false`, `release_allowed=false`, `live_1c_allowed=false`.
- Реализация: физические Дт/Кт Инталев разрешаются отдельным fail-closed resolver в признанный расходный счёт, сторону и `economic_block`; `presentation_block` и полный `report_placement` сохраняются отдельно. Legacy `article_info.block` теперь является совместимым финансовым alias доказанного `economic_block`, а automatic target selection, approval scope и payroll target grouping явно потребляют `economic_block`. `catalog_block`, expense-account status/side и presentation diagnostics выведены в cross-journal evidence. Manual `APPROVED_EXACT_SCOPE` authority не изменялась и не участвовала в fixture/smoke; R005-021 approval semantics остаются вне scope.
- GREEN: focused R005-024 — `6/6 PASS`, включая debit `26`, debit `44.1`, credit-side `44.3`, сохранение существующего `44.2 => Расходы на складскую логистику`, no-expense и conflicting-expense. Затронутый cross-journal контур — `28/28 PASS`. Полный R005 Node-прогон последовательно по одному процессу на test file — `42/42 files PASS`, `260 PASS / 1 existing opt-in SKIP / 0 FAIL`.
- Fresh Sakhalin smoke: новый isolated run `run_r005024_20260902_b`, `ERP-000000076 / 3 Сахалин / 2025-01`, завершил owner-wrapper R005 с `READY_REPORT_ONLY` cross-journal evidence. Для exact ERP `SourceRowID=2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825`, строка `1617`, сумма `1854`: Intalev `SourceRowID=49F314B1C167A69C21DAB409F8226D71FEAC1B1B2517848408888842D07A272A`, строка `4082`, expense account `26`, side `DEBIT`, `economic_block=административные расходы`, `presentation_block=расходы на складскую логистику`; выбран target `Административные расходы / Проезд/доставка сотрудников`, code `00-000067`, account `Счет затрат 26`, status `PROVEN_UNIQUE_TARGET_IN_INTALEV_BLOCK`.
- Корректировка для `1854` остаётся экономически требуемой: physical ERP row `1617` / exact SourceRowID использован одной доказанной парой; STORNO `-1854` с source classification `расходы на транспортную логистику`, REPOST `+1854` на `Административные расходы / Проезд/доставка сотрудников / 00-000067`; net `0`. `reused_intalev_rows=0`, `reused_erp_rows=0`.
- Smoke counters/safety: `intalev_scoped_rows=9284`, `erp_scoped_rows=3132`, `unique_pairs=330`, `approved_balanced_pairs=38`, `financial_pair_rows=76`; `posting_rows=0`, `live_rows=0`, `executed_rows=0`, `report_only=true`, `execution_allowed=false`, `live_1c_allowed=false`, `ready_to_upload=false`, `release_allowed=false`, `physical_row_reuse_blocked=true`. `rules_service=false` остаётся каноническим invariant; скрытый rules service не запускался и не возвращался.

### R005-025 — доказанная cross-journal операция отсутствует в раскрываемом основном дереве

- Дата: `02.09.2026`
- Статус: `CLOSED`
- Причина закрытия: PR #39 объединён в `release/r17`; Issue #35 закрыт со статусом `COMPLETED`.
- Сообщил: пользовательская карточка `R005-025_MAIN_TREE_CROSS_JOURNAL_SOURCE_DRILLDOWN`, Issue #35.
- Наблюдаемое поведение: физическая ERP-операция может быть полностью доказана в `crossJournalEvidence`, `04A_Расхождения_проводок` и `04B_R001_решения`, но отсутствовать как раскрываемая дочерняя строка `OPERATION` на основном листе `01_Сверка_дерево`. Якорь: ERP `SourceRowID=2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825`, строка `1617`, документ `Трансляция 0000001782 от 19.03.2026 9:03:39`, проводка `8`, дата `31.01.2025`, сумма `1854`, организация `ПВ`, Дт `26`, Кт `76.5`.
- Ожидаемое поведение: доказанная физическая ERP-операция отображается непосредственно под единственной детерминированно связанной reconciliation/presentation-строкой основного дерева как скрытая при свёрнутом состоянии дочерняя evidence-строка. Она показывает исходный `SourceRowID`, физическую строку, дату, регистратор, номер проводки, счета, аналитики, подразделения, сумму, организацию и статус доказательства, но не меняет сумму, дельту, статус, целевой блок/статью, коррекционную пару либо финансовую authority родителя.
- Доказательство привязки: классификация `EXISTING_STABLE_PRESENTATION_IDENTITY`. У доказанной cross-journal строки `intalev_report_node_id=INTALEV:d574fd8ed9203004329907d32080e6a002958ebcdbd75f84aaa23e1c43dabbfe`, статус `PROVEN_LIVE_REPORT_LEAF_ANALYTIC`; этот ID равен единственному `hierarchy_node_id` source-driven/main-tree presentation-узла и `presentation_structural_proof.node_id`. Текущие S-коды и Excel-строки являются только наблюдаемым evidence и не входят в binding.
- Затронутые пункты контракта: §§4–7, 9–10, 12.1, 12.4, 13–14; A01–A12, A22, A25, A30–A32. Канонический контракт v0.5 и его SHA-256 не изменяются.
- Новое или уточнённое требование: `crossJournalEvidence` проецируется в существующую operation-tree presentation только при доказанном статусе и exact one-match по стабильному presentation identity. Article-only/fuzzy matching, первый S-row, Excel row number, hard-coded S-code, amount или target economic block запрещены. Один физический ERP `SourceRowID` отображается под одной presentation-строкой ровно один раз независимо от STORNO/REPOST; presentation-dedup не меняет финансовый no-reuse guard. Неуникальная либо отсутствующая binding остаётся unbound/review.
- Обязательный регрессионный тест: actual generated XLSX до production fix содержит доказанную cross-journal строку, но не содержит её bound `OPERATION` child в `01_Сверка_дерево`; после fix exact parent и child присутствуют в одной outline-группе, parent имеет collapsed state, child hidden state и точные физические поля. Дополнительно проверяются одинаковый article text в разных ветвях, unproven candidate, no-unique binding, STORNO/REPOST dedup и сохранение `39` legacy unassigned operation rows.
- PASS: свежий Sakhalin `2025-01` сохраняет R005-024 financial result `1854`, Intalev row `4082`, account `26`, economic block `Административные расходы`, presentation block `Расходы на складскую логистику`, target `Административные расходы / Проезд/доставка сотрудников / 00-000067 / Счет затрат 26`, STORNO/REPOST `-1854/+1854`, net `0`; основной XLSX содержит exact physical child под unique identity; legacy operations не исчезают; `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live rows `0`, execution/upload/release/live/delete flags `false`, ERP financial reuse `0`.
- Реализация: `projectCrossJournalSourceDrilldown` является малым presentation-only адаптером: он принимает только `UNIQUE_PAIR` с `PROVEN_LIVE_REPORT_LEAF_ANALYTIC`, сохраняет физические поля ERP и связывает evidence с presentation row исключительно по exact one-match `intalev_report_node_id=hierarchy_node_id`. Child маркируется `ERP_SOURCE_EVIDENCE`, `excluded_from_totals=true`, `correction/posting/live/executed=0`; дедупликация выполняется по `(resolved parent identity, ERP SourceRowID)`. Неуникальные/отсутствующие identity остаются unbound. Финансовый matching, economic block, target, correction rows, R001 и legacy `operationEvidence` не изменялись.
- Протокол проверки: обязательный actual-XLSX RED до production mutation — `0/2 PASS`: document child отсутствовал (`null`), physical child count был `0`; после реализации focused GREEN — `2/2 PASS`. Напрямую затронутые suite — `54/54 PASS`; полный последовательный R005 Node — `43/43 files PASS`, `262 PASS / 1 existing opt-in SKIP / 0 FAIL` (`263` tests). Syntax изменённых production/test modules, `git diff --check` — `PASS`. Fresh owner-wrapper smoke `run_r005025_20260902_b`, `ERP-000000076 / 3 Сахалин / 2025-01`, workbook SHA-256 `F5388837ED8FB753AC01971377F5A658B1B24424BF23C1A18DFBD41FC3B3DD2C`: anchor в `01_Сверка_дерево` ровно один раз, parent Excel row `269` (`outlineLevel=5`, `collapsed=1`, visible), child Excel row `272` (`outlineLevel=6`, `hidden=1`), `outlinePr summaryBelow=0`, `sheetView showOutlineSymbols=1`; `04A` row `228`, `04B` rows `19/20`, STORNO/REPOST `-1854/+1854`, net `0`. Drilldown audit: `69` children / `69` unique physical SourceRowID / `0` duplicates, `2` fail-closed unbound; legacy unassigned `39`. Safety: `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live `0`, execution/upload/release/live/delete `false`, ERP financial reuse `0`.

### R005-027 — валидный Service handoff понижает доказанные Сахалинские пары до СПОРНО

- Дата: `02.09.2026`
- Статус: `VERIFIED`
- Сообщил: Issue #43, карточка `R005-027_SAKHALIN_READY_AUTHORITY`.
- Наблюдаемое поведение: свежая Service-цепочка `ERP-000000076 / 3 Сахалин / 2025-01` создаёт в R005 `38` физически доказанных сбалансированных пар / `76` строк с `reused_erp_rows=0`, а immutable handoff повторно проверяет scope, source hashes, R005 hashes, точный набор SourceRowID и reuse `0`. Но R001 создаёт `38` group-scoped пар как `READY=0 / SPORNO=76`.
- Ожидаемое поведение: только после валидации exact Service handoff уже доказанная R005-пара с одной физической ERP-строкой, однозначной целью, равными по модулю STORNO/REPOST и нулевым итогом сохраняет финансовую authority в защищённом R001. Недоказанная, неоднозначная, повторно использованная или несбалансированная пара остаётся СПОРНО/блокируется.
- Доказательство: Issue #42 run `run_5e4e3b33d8e27f22855033c8`, context `ctx_1547691f7f326ba92a55c2d4`; Intalev SHA-256 `AE230FB3E960D1E36C0D2288A33831D6D4D74772C769E7603D6E20916DB49DE0`, ERP SHA-256 `B2F97D3A7F320EE3BE3A62D0423D4BFB7A215ED92D7ED6A1A771FC04EBCF89D1`, handoff SHA-256 `E0B01A833248B85E455959F2EBA5D93B7BB58495C149E367F580BE7D41F13DAA`. R005: `38/38` STORNO/REPOST, `-829993.38/+829993.38/0`; R001: `76` canonical rows, `READY=0`, `SPORNO=76`.
- Классификация первопричины: `MULTIPLE_GATES`. Gate 1 / первая точная потеря authority — `modules/corrections/source/r001_group_scoped_materialization.mjs::caseFor()`: после успешных `exactFinancialAuthority`, unique target, exact physical operation и exact amount функция безусловно записывает `correction_allowed=false`, `correction_authority=GROUP_SCOPED_ARTICLE_REPLACEMENT`, `output_route=SPORNO`.
- Gate 2 / следующий material blocker после исправления Gate 1: `modules/corrections/source/r001_materialization_contract.mjs::assertReadyPhysicalSource()` сначала проверяет всю canonical physical identity и точно три analytics slots, но затем дополнительно запрещает пустое фактическое значение любого слота. Реальная anchor-строка имеет `debit_analytics=["Проезд/доставка сотрудников","",""]` и `credit_analytics=["Служебный","",""]`, поэтому получает `READY_PHYSICAL_IDENTITY_INCOMPLETE`. Это противоречит §12.3: отсутствующие фактические субконто остаются пустыми, а §12.4 запрещает их додумывать. Все canonical identity fields, exact three-slot array shape, READY proof flags, STORNO tuple, handoff exact-set/no-reuse/pair-shape/balance guards остаются обязательными.
- Gate 3 / Service package boundary после исправления Gate 1 и Gate 2: fresh task-HEAD R001 manifest уже содержит exact `76 READY / 0 SPORNO`, `38/38` STORNO/REPOST и `canonical_output_integrity.result=PASS`, но Service завершает стадию как `FAILED / R001`. Первая точная потеря на этом уровне — `service/source/report_workbook_validation.go::executableCorrectionAuthority()`: allowlist не знает новый узкий authority label `SERVICE_HANDOFF_GROUP_SCOPED_PHYSICAL_AUTHORITY`, поэтому `validateMaterializationRouting()` отклоняет доказанную READY-строку как `lacks proven correction authority`. Допустимо добавить только это exact значение; похожие, пустые и произвольные authority остаются запрещены, а все проверки proof, physical identity, SHA, SourceRowID A:AA equality и route counts сохраняются.
- Gate 4 / следующий Service workbook blocker после exact Gate-3 allowlist: fresh R001 снова имел `76 READY / 0 SPORNO` и integrity `PASS`, но фактическая строка `10_R001_Материализация` несла `Proof status=GROUP_SCOPED_ARTICLE_REPLACEMENT_PROVEN`, тогда как `validateMaterializationRouting()` принимает для READY только exact `PROVEN` в том же compound guard. Исправление выполнено у producer: только при уже доказанном `readyAuthority` `caseFor()` пишет канонический READY-token `PROVEN`; no-handoff/SPORNO сохраняет `GROUP_SCOPED_ARTICLE_REPLACEMENT_PROVEN`. Service не принимает group-scoped proof как самостоятельное READY-доказательство.
- Структурный control не является первопричиной и остаётся нефинансовым: `PACKAGED_USER_CSV_NO_EXACT_SCOPE`, `set_count=0` и `correction_authority=false` не дают и не отнимают financial authority. Generic `BLOCKED_PROFILE_REVIEW_REQUIRED` также не подменяет exact financial proof.
- Затронутые пункты контракта: §§6–7, 10, 12.2–12.4, 13–14; A07, A08, A10, A22, A25, A30, A32. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A`, не изменяется.
- Допустимый scope: exact R001 authority seam между verified handoff и group-scoped materialization, focused regression, напрямую затронутые Service/handoff/R001 tests и эта запись. Запрещены hard-code Сахалина/строки/SourceRowID в production, добавление Сахалина в packaged CSV, финансовая authority для structural controls, обход handoff, ослабление physical/no-reuse/balance guards, R005-024/R005-025/R005-026 и live 1C/upload/posting.
- Обязательный RED: Service/portable-equivalent fixture для `3 Сахалин / 2025-01` с no-exact/no-settings structural selection, доказанной физической парой и точным SourceRowID в handoff первой проверкой ожидает `materialization_case.correction_allowed=true`, затем `output_route=READY`; до исправления получает `false` / `SPORNO`. Отрицательные проверки сохраняют СПОРНО/блокировку для incomplete proof, duplicate SourceRowID и unbalanced pair; structural no-settings остаётся нефинансовым, `service-none` semantics не меняются.
- Фактический RED на exact base `8974b1decab9c0b6936732f012056cfe4ace45e3`: `modules/corrections/source/r005_027_sakhalin_ready_authority.test.mjs` — `0 PASS / 1 FAIL`; первая exact authority assertion получила `materialization_case.correction_allowed: false !== true` уже после `MATERIALIZED_GROUP_SCOPED_STORNO_REPOST`, до проверки финальных READY/SPORNO counts. Production до RED не изменялся.
- Фактический RED Gate 3 после доказанных Gate 1/2 и до Service production mutation: `go test -count=1 -run '^TestR005027ServiceValidatorAcceptsExactGroupScopedHandoffAuthorityOnly$' .` — `FAIL`; первая assertion получила `READY materialization row 5 lacks proven correction authority` для exact `SERVICE_HANDOFF_GROUP_SCOPED_PHYSICAL_AUTHORITY` при полной PROVEN/physical/A:AA строке.
- Фактический RED Gate 4 после exact Gate-3 allowlist и до producer mutation: та же focused Service-проверка с фактическим `Proof status=GROUP_SCOPED_ARTICLE_REPLACEMENT_PROVEN` — `FAIL`, `READY materialization row 5 lacks proven correction authority`. Контрактная классификация сохраняется как `MULTIPLE_GATES`; нормализация выполняется у producer только для уже доказанного READY, Service не начинает принимать group-scoped proof token как самостоятельное READY-доказательство.
- PASS: fresh real Сахалин R005 → validated immutable handoff → protected R001 сохраняет exact `38` пар / `76` READY-строк, `-829993.38/+829993.38/0`, точный SourceRowID set и reuse `0`; anchor `1854` и R005-025 drilldown не меняются. `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live `0`, execution/upload/release/live/delete flags `false`.
- Реализация: Gate 1 передаёт только verified handoff SourceRowID set в group-scoped materialization и сохраняет READY authority лишь для exact member после всех прежних financial/target/physical/amount guards; Gate 2 разрешает пустое фактическое значение отдельного субконто, сохраняя обязательные explicit arrays ровно из трёх слотов и всю canonical identity; Gate 3 добавляет в Service executable-authority allowlist только exact `SERVICE_HANDOFF_GROUP_SCOPED_PHYSICAL_AUTHORITY`; Gate 4 нормализует proof в `PROVEN` только для уже доказанного READY. Prefix/wildcard, plain `GROUP_SCOPED_ARTICLE_REPLACEMENT`, неизвестные authority, non-PROVEN READY, SPORNO с executable authority, incomplete physical identity, bad SHA, SourceRowID mismatch/reuse и unbalanced pairs остаются fail-closed.
- Протокол проверки: исходный focused RED — `0 PASS / 1 FAIL`, первая потеря `correction_allowed false !== true`; Gate-3 RED — exact authority отклонён Service как `lacks proven correction authority`; Gate-4 producer RED — actual `GROUP_SCOPED_ARTICLE_REPLACEMENT_PROVEN !== PROVEN`. Финальный focused Node — `32/32 PASS`; affected-10 — `94/94 PASS`; полный последовательный R001 — `23/23 files PASS`, `237 PASS / 1 existing opt-in SKIP / 0 FAIL` (`238` tests); полный последовательный R005 — `43/43 files PASS`, `262 PASS / 1 existing opt-in SKIP / 0 FAIL` (`263` tests); полный Service Go `go test -count=1 ./...` — `PASS`; syntax изменённых MJS и `git diff --check` — `PASS`.
- Fresh final Service smoke: run `run_a52093bac814a5845a59a599`, context `ctx_078cd1a57558fd94722a0313`, `COMPLETED_REPORT_ONLY / DONE`. R005 сохраняет ожидаемый `BLOCKED_PROFILE_REVIEW_REQUIRED`, structural selection `SERVICE_NO_SETTINGS` остаётся нефинансовым; handoff содержит exact `38` unique SourceRowID, reuse `0`, все cross-checks `true`. R001: `76/76 READY`, `0 SPORNO`, `38/38` STORNO/REPOST, `-829993.38/+829993.38/0`, canonical integrity `PASS`. Anchor SourceRowID `2748C4E6F44E8C9273AFE32A3D5B36DE99C1D6FD39FA972B3976D7550059C825`: main-tree row `272` под row `269`, ERP row `1617`, posting `8`, amount `1854`, Dt/Kt `26/76.5`, Intalev row `4082`, target `00-000067 / Счет затрат 26`, STORNO/REPOST/net `-1854/+1854/0`; XML: row `269` `outlineLevel=5/collapsed=1`, rows `270–272` `outlineLevel=6/hidden=1`. Safety: `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live `0`, execution/ready-to-upload/release/live/delete flags `false`.
### PACK-005 — финальный portable release manifest не подтверждает release identity и ZIP

- Дата: `02.09.2026`.
- Статус: `OPEN`.
- Сообщил: Issue #44, `PACK-005_FINAL_RELEASE_ATTESTATION`.
- Наблюдаемое поведение: `R17_PACKAGE_MANIFEST.json` создаётся до финального ZIP и не связывает в одном каноническом объекте `source_branch=release/r17`, точный source/contract, EXE и фактические size/SHA-256 финального ZIP. Текущий verifier возвращает ZIP size/SHA только в отчёте проверки и не требует отдельного post-archive attestation.
- Ожидаемое поведение: после появления финальных байтов ZIP builder создаёт внешний детерминированный `R17_RELEASE_MANIFEST.json`, который связывает source branch/head/inventory, contract v0.5/SHA, policy/toolchains, SHA embedded manifest, EXE path/size/SHA, ZIP name/size/SHA и закрытую safety identity. Verifier требует этот объект, пересчитывает фактические байты и fail-closed отклоняет пропуск либо подмену каждого обязательного поля. Финальный ZIP hash не встраивается внутрь того же ZIP из-за самоссылочного цикла.
- Затронутые пункты контракта: §§1, 13, 14, 16; переносимый локальный EXE, `REPORT_ONLY=true`, `rules_service=false`, posting/executed/live rows `0`, запрет автоматической загрузки/проведения, обязательные manifest/SHA-256/test protocol. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A`, не изменяется.
...
- Дата: `03.09.2026`.
- Статус: `VERIFIED`.
- Сообщил: карточка `R17_PACKAGING_R005_CATALOG_PIN`.
- Наблюдаемое поведение: `IntegratedRuntimePrepareTest.test_r005_catalog_repository_sources_match_pins_after_rebase` и соответствующий packaging map содержали четыре устаревших pin: `modules/reconciliation/source/opiu_reconcile.mjs` — expected `1D7031C36CD337B5ACC31B357CE7B40ACDB2C1F05BDCA4005D158C2FC0D89B6`, actual `66C2587238D4CD2BACE280AD857E917D088851228477C547702D2F247328FD80`; `modules/reconciliation/source/intalev_catalog_binding.mjs` — expected `846C1A2BBFC8E7CDA577FC151A23E039FF3A36281F7EC488EC63D927525EAEAF`, actual `DB7D3B97711227ACDE8A1D5396EFE919A4E4042563C2C1A688D50F05247DE4B4`; `modules/reconciliation/source/intalev_catalog_binding.test.mjs` — expected `AA7C43700E55B85B4A7B3B2E9BD469651C455F0704F0DD02BD51D6A295EA6E73`, actual `5ABCF2632CC14AE559282E84AE6A5EA4F9795AB00946227CA8F32E1E562888E6`; `modules/reconciliation/source/intalev_catalog_workbook_semantics.test.mjs` — expected `7E40F283A8AF4DB1B6EDD32704EC24671A0B75634C3D972BDBFEE45AE0AD4811`, actual `00C55C1B4671FA7722F5084BE37E63BAFE74B3F62406A836C067037C28B22F82`. `modules/reconciliation/source/reference_catalog_manifest.mjs` already matched: `85094D961FE12895A122B1EEE5EF9DC502D46E791A4406921FB2158B993C6748`.
- Ожидаемое поведение: packaging pin совпадает с фактическими байтами current authoritative R005 source; последующий drift отклоняется fail-closed.
- Доказательство authority: все пять источников tracked в Git и находятся непосредственно в `modules/reconciliation/source/`. Старые четыре pin появились в baseline commit `7e1af5f68a599d90fba0a207b58a0a6a0b27242e`; принятые R005 source changes в `e1fa765ac07ee2fc2f7498acb66fa1c76a4c9685`, `b9bba18157a5149e695b6dfd15ea65262c886e5` и `89c6043d40ed9608b16c62c596788c273ddb70bb` входят в exact `ddd3057d12429a17a95fc5ac04ee67edb563cf93` и `release/r17`. Прямой SHA-256 текущих tracked bytes совпал со всеми пятью map values.
- Подтверждённая первопричина: `OBSOLETE_PIN`; источник изменён принятой R005 catalog implementation, а packaging pin не был обновлён. Source/business contents не изменяются.
- Затронутые пункты контракта: §§1, 6, 12.4, 13–14, 16 и Приложение B; `REPORT_ONLY=true`, `rules_service=false`, автоматическая загрузка/проведение в 1С запрещены. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A`, не изменяется.
- Scope: только соответствующие R005 catalog packaging pins/tests и эта запись; PACK-006, PACK-007, ExactOwner, ignored Excel inventory, R005 financial/business source и release/r17 вне scope.
- Обязательный RED до production mutation: `python -m pytest -q service/packaging/test_prepare_integrated_review_runtime.py -k "test_r005_catalog_repository_sources_match_pins_after_rebase"` — `1 failed, 7 deselected`; assertion сообщил actual `DB7D3B97711227ACDE8A1D5396EFE919A4E4042563C2C1A688D50F05247DE4B4` против expected `846C1A2BBFC8E7CDA577FC151A23E039FF3A36281F7EC488EC63D927525EAEAF`.
- GREEN: тот же targeted test после pin refresh — `1 passed, 7 deselected`; related `-k "r005_catalog"` — `3 passed, 5 deselected`; hash-drift negative mutation — `1 passed, 7 deselected`, mismatch отклоняется как `R005_CATALOG_OVERLAY_SOURCE_HASH_MISMATCH`.
- Обязательный broader run: `python -m pytest -q service/packaging` — `210 passed, 14 failed, 2 skipped, 2 subtests passed`. Оставшиеся failures не являются PACK-008: один `RUNTIME_OVERLAY_WORKING_TREE_MISMATCH:missing=0:extra=67` (ignored Excel inventory), пять setup failures из-за отсутствующего `modules/rules-engine/source/adapters/r005_adapter.mjs` и три `RULES_OVERLAY_SOURCE_MISSING:modules/rules-engine/source/adapters/r005_adapter.test.mjs` (PACK-006 rules-overlay), четыре отсутствующих внешних `C:\Users\NB-FIT\development\OPIU_1.9.4` provenance source и один `INTEGRATION_OVERLAY_BLOB_READ_ERROR` из-за repo-root (exact-owner/repo-root). Нового PACK-008 failure нет.
- PASS: изменены только необходимые packaging pin definitions, соответствующий scoped test isolation и эта запись; tracked R005 source bytes, contract, safety и финансовая логика не изменены; `git diff --check` проходит.
...
### PACK-007 — user-delivery packaging привязан к внешнему OPIU_1.9.4 пути и устаревшей provenance

- Дата: `03.09.2026`.
- Статус: `VERIFIED`.
- Сообщил: карточка `R17_PACKAGING_OPIU_194_DELIVERY_PROVENANCE`.
- Наблюдаемое поведение: `service/packaging/build_user_delivery_candidate.py` вычисляет `REPO_ROOT` через `Path(__file__).resolve().parents[4]`; builder и его тесты обращаются к developer-specific `development/OPIU_1.9.4`. На clean exact base `ddd3057d12429a17a95fc5ac04ee67edb563cf93` targeted RED даёт `4 failed, 15 passed, 2 subtests passed`: `BACKEND_PROVENANCE_MISSING:BASELINE_PROVENANCE.json`, `UI_PROVENANCE_MISSING:BASELINE_PROVENANCE.json`, `FileNotFoundError` для `correction_engine_r001.mjs` и runtime pin resolution failure.
- Доказательство authority: текущий repository-owned source лежит в `modules/corrections/source/**`; backend provenance — `service/backend_overlay/BASELINE_PROVENANCE.json` (17 overlay files с совпадающими фактическими hashes); UI provenance — `service/ui_overlay/BASELINE_PROVENANCE.json`. Contract v0.5 требует локальный переносимый пакет и не требует внешнего developer workspace. Existing builder constants/pins относятся к отсутствующему смешанному historical snapshot; UI provenance также содержит overlay hashes, не совпадающие с фактическими tracked UI bytes.
- Ожидаемое поведение: repository root и source/overlay authority разрешаются детерминированно от текущего checkout; developer-specific absolute paths не нужны; current provenance и current source pins проверяются fail-closed. Missing authoritative provenance, wrong provenance metadata и wrong source hashes по-прежнему дают отказ.
- Классификация первопричины: `STALE_MACHINE_PATH_AND_MIXED_PROVENANCE`.
- Затронутые пункты контракта: §§1, 6, 12.4, 13–14, 16 и Приложение B; A06–A07, A10, A14, A32. Канонический контракт v0.5, SHA-256 `B2C7D11B8373E603D0FA0C9B9AF090CF3026085A4E80457B228336CEA3DFAB5A`, не изменяется.
- Обязательный регрессионный тест: clean/portable checkout без `NB-FIT`-specific source path разрешает current source pins и backend/UI provenance; подмена либо отсутствие authoritative provenance и подмена source hash отклоняются fail-closed. PACK-006 rules-overlay cluster на PACK-007-only branch является `OUT OF SCOPE` и не заявляется как GREEN или fixed.
- Safety: `REPORT_ONLY=true`, `rules_service=false`, `release_approved=false`, автоматическая загрузка/проведение в 1С запрещены; финансовая логика и physical ERP evidence не изменяются.
- Реализация: ветка `fix/packaging-delivery-provenance` разрешает repository root через `parents[2]`, использует tracked `modules/**` и `service/{backend_overlay,ui_overlay}/**`, обновляет пины до exact current bytes и исключает только retired JavaScript Rules-engine paths из user-delivery runtime pin set. Existing UI provenance record приведён в соответствие с exact tracked overlay bytes; no synthetic source or provenance file created.
- Протокол проверки normalized PACK-007-only branch: clean exact-base RED — `4 failed, 15 passed, 2 subtests passed`; targeted PACK-007 GREEN — `22 passed, 2 subtests passed`; broader packaging suite (`python -m pytest -q service/packaging`) — `215 passed, 12 failed, 2 skipped, 2 subtests passed`. Remaining failures are OUT OF SCOPE: 9 PACK-006 Rules-overlay expectations, 1 exact-owner/repo-root, 1 ignored Excel inventory mismatch (`missing=0:extra=67`) и 1 obsolete R005 catalog pin. No new PACK-007 failure. `git diff --check` — PASS.
