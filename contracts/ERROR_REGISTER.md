...
### TRANSFER-001 — source/GK role resolution collapses when source organization is `ГК`

- Дата: `30.08.2026`
- Статус: `VERIFIED`
- Сообщил: пользовательский review PR #9, Issue #2.
- Наблюдаемое поведение: `validate_source_effect()` определяет source- и GK-роли только по `document_organization`. При `balance.organization == config.manager_organization` обе пары имеют организацию `ГК`, поэтому валидация видит по две строки каждой роли, не разрешает source/GK posting и блокирует иначе `ACTIONABLE` остаток.
- Ожидаемое поведение: роли определяются по структуре проводки и approved direction: source-role содержит source account `79.2`/`79.3` на стороне, обнуляющей исходный остаток, с противоположной стороной `79.1`; GK-role содержит две `79.1`, GK financial department и точное approved supplier direction. Для каждой ACTIONABLE нормализованной строки создаются ровно две PostingRows, даже если source organization равна `ГК`.
- Доказательство: карточка `FIX_PR9_REVIEW_BLOCKER`, старый HEAD `1e29c5efaa4b2b594302df63ac0a6c3260d1527c`, воспроизводимый сценарий `ГК` × (`79.2`/`79.3`) × (Дт/Кт).
- Затронутые пункты контракта: PRODUCT_CONTRACT §§4–7, 10; критерий Issue #2 о двух PostingRows, exact direction, Decimal amount, traceability, zero ending balance и single consumption; текущий DOCX-контракт §§1.1–1.2, 3.1, 4.1–4.2, 13.
- Новое требование: существующие source-effect проверки не ослабляются; добавляется обязательное различение posting role, независимое от совпадения имён организаций.
- Обязательный регрессионный тест: четыре `ГК`-as-source кейса (`79.2` Дт/Кт, `79.3` Дт/Кт) остаются ACTIONABLE, `generate_transfer()` возвращает две строки, source-effect проходит, обе стороны после применения равны `Decimal(0)`, суммы точны и обе роли соответствуют PRODUCT_CONTRACT.
- Реализация: `validate_source_effect()` разрешает source-role по направлению `79.x ↔ 79.1`, а GK-role по паре `79.1/79.1`, финансовому департаменту и supplier direction; `document_organization` остаётся в exact direction/trace validation и не используется как единственный role discriminator. Неподдержанный `TransferConfig.rules_version` блокируется до построения PostingRows.
- Протокол проверки: commits `5910e767a46eee640676f5e631d175fa070a70a7` и `d455ea09fe62a1e47c5da663fb022170da21dd9c`; `python -m pytest -q` — `33 passed`; `python -m ruff check .` — PASS; Product CI run `33297358171` — success.
