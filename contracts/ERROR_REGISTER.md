...
- ROOT CAUSE: первая потеря происходила в R001 после принятого Service handoff: workbook adapter не восстанавливал иерархический уровень строк legacy-дерева, поэтому доказанные source-строки не попадали в group-scoped materialization; paired-liability authority дополнительно ограничивалась только direct-parent узлом. После восстановления 32 строк canonical output Service run-scoped validator ошибочно открывал hardcoded `reconciliation.xlsx`, хотя подписанный output registry канонически регистрировал `Сверка.xlsx`.
...
- Реализация: adapter нормализует legacy hierarchy labels/levels; correction engine допускает только source-bearing hierarchy roles и выдаёт physical-source reuse proof лишь для уникального source текущего run; paired-liability authority ищет единственный exact source в доказанном business block; materialization сохраняет physical proof. Go Service выбирает root-level reconciliation workbook из проверенного manifest output registry (`reconciliation.xlsx` или `Сверка.xlsx`) вместо hardcoded имени.
- Протокол проверки: exact October test — `1/1 PASS`; focused R001 — `19/19 PASS`; все October tests — `245 PASS / 1 SKIP / 0 FAIL`; полный R001 Node — `267/267 PASS`; полный R005 Node — `242 PASS / 1 SKIP / 0 FAIL`; service/web Node — `54/54 PASS`; pinned Go `go1.22.12` targeted direct handoff — `PASS`; полный `go test -count=1 ./...` и `go vet ./...` — `PASS`; production syntax `99` files — `PASS`; `git diff --check` — `PASS`.
- Acceptance: независимый source-Service run `run_844900bdd43f81d2e6016e1c` завершён `COMPLETED_REPORT_ONLY / DONE`; `32` canonical/draft/materialized, `16/16` STORNO/REPOST, `16` пар, суммы `-364066/+364066/net 0`, три `_СПОРНО` workbook `10/14/8`, R005 `21/21` unique proven physical source rows, canonical workbook/registry/manifest integrity `PASS`; `posting_rows=0`, execution/upload/release/live flags `false`.
